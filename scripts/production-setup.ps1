...
Write-Host "Working in project directory: $projectRoot" -ForegroundColor Green
...
"@
        ...
        if (Test-Path "C:\tools\nginx\conf\") {
            ...
            Write-Host "💾 Backup saved to: config\nginx.conf" -ForegroundColor Gray
        }
    } catch {
...
$prodEnvConfig | Out-File -FilePath "config\.env.production" -Encoding UTF8
...
if (Test-Path "config\.env.production") {
    Copy-Item "config\.env.production" ".env" -Force
    Write-Host "✅ Production environment loaded" -ForegroundColor Green
} else {
    Write-Host "⚠️ Production environment file not found. Using default .env" -ForegroundColor Yellow
}
...
$startupScript | Out-File -FilePath "scripts\start-production.ps1" -Encoding UTF8
...
$monitorScript | Out-File -FilePath "scripts\monitor-health.ps1" -Encoding UTF8
...
Write-Host "   .\scripts\start-production.ps1" -ForegroundColor Gray
